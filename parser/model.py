...
class CRFSemanticRoleLabelingModel(Model):
    ...
    def forward(self, words, feats=None):
        ...
        Returns:
            ~torch.Tensor, ~torch.Tensor, ~torch.Tensor, ~torch.Tensor, ~torch.Tensor:
                ...
                and possible roles on each edge of shape ``[batch_size, seq_len, seq_len, n_roles]``.
    ...
    def loss(self, s_edge, s_role, edges, roles, mask):
        ...
        if prd_mask.any():
            ...
            edge_dist, role_dist = CRFSRL(s_edge, mask[s_mask]), CRFSRL(s_edge + s_role, mask[s_mask])
    ...
    def decode(self, s_edge, s_role, mask):
        ...
        if prd_mask.any():
            ...
            s_edge[..., 0].masked_fill_(prds.unsqueeze(-1).ne(prds.new_tensor(range(mask.shape[1]))), MIN)
            edge_preds[prd_mask] = CRFSRL(s_edge + s_role, mask[s_mask]).argmax
