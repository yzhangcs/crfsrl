...
from .model import CRFSemanticRoleLabelingModel
...
class CRFSemanticRoleLabelingParser(Parser):
    ...
    def _train(self, loader):
        ...
        for i, batch in enumerate(bar, 1):
            words, *feats, arcs, edges, roles, spans = batch
    ...
    def _evaluate(self, loader):
        ...
        for batch in loader:
            ...
            from supar.structs import CRFDependency
            from supar.utils.fn import ispunct
            arc_preds = CRFDependency(s_edge, mask).argmax
            ...
            metric([[(i[0], *i[2:-1], self.ROLE.vocab[i[-1]]) for i in s if i[-1] != self.ROLE.unk_index] for s in role_preds],
                   [[i for i in s if i[-1] != 'O'] for s in spans], arc_preds, arcs, mask)
    ...
    def _predict(self, loader):
        ...
        for batch in progress_bar(loader):
            ...
            if self.args.syntax:
                arc_preds = CRFDependency(s_edge, mask).argmax
                preds['arcs'].extend([i.tolist() for i in arc_preds[mask].split(lens.tolist())])
    ...
    def build(cls, path, min_freq=7, fix_len=20, **kwargs):
        ...
        transform = CoNLL(FORM=(WORD, CHAR, ELMO, BERT), LEMMA=LEMMA, POS=TAG, HEAD=ARC, PHEAD=(EDGE, ROLE, SPAN))
