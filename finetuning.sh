# nohup bash finetuning.sh config=configs/conll05.crf.srl.bert.ini    path=exp/conll05.crf.srl.bert    bert=bert-large-cased train=data/srl/conll05/train.conllu dev=data/srl/conll05/dev.conllu test=data/srl/conll05/test.conllu ood=data/srl/conll05/brown.conllu devices=4,5,6,7 > log.conll05.crf.bert    2>&1 &
...
predict() {
    ...
    for dataset in dev test ood; do
        for seed in {0..3}; do
            ...
            printf "nohup python -u crf.py predict -c $config -d ${device_arr[$device_id]} -p $path/model.batch$batch.dropout$dropout.epochs$epochs.rate$rate.$seed --bert=$bert --data ${!dataset} --pred $path/$dataset.$seed.pred.conllu 2>$path/model.batch$batch.dropout$dropout.epochs$epochs.rate$rate.$seed.$dataset.predict.log.verbose &\n\n"
            nohup python -u crf.py predict -c $config -d ${device_arr[$device_id]} -p $path/model.batch$batch.dropout$dropout.epochs$epochs.rate$rate.$seed --bert=$bert --data ${!dataset} --pred $path/$dataset.$seed.pred.conllu >$path/model.batch$batch.dropout$dropout.epochs$epochs.rate$rate.$seed.$dataset.predict.log.verbose &
            pids[${i}]=$!
            echo ${pids[*]}
        done
    done
    for dataset in dev test ood; do
        for seed in {0..3}; do
            ...
            nohup python -u crf.py predict -c $config -d ${device_arr[$device_id]} -p $path/model.batch$batch.dropout$dropout.epochs$epochs.rate$rate.$seed --bert=$bert --data ${!dataset} --pred $path/$dataset.$seed.pred.gold.conllu --prd >$path/model.batch$batch.dropout$dropout.epochs$epochs.rate$rate.$seed.$dataset.prd.predict.log.verbose &
            pids[${i}]=$!
            echo ${pids[*]}
        done
    done
